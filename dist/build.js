...
function execute(context) {
  ...
  recursive(chcpContext.sourceDirectory, ignore, function (err, files) {
    var hashQueue = prepareFilesHashQueue(files, chcpContext);
...
function prepareFilesHashQueue(files, chcpContext) {
  var queue = [];
  for (var i in files) {
    var file = files[i];
    if (!hidefile.isHiddenSync(file)) {
      queue.push(hashFile.bind(null, file, chcpContext));
    }
  }
